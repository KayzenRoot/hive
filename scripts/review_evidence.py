...
# WO-029-C2: progressive-disclosure domain classification correction.
# Modern domain names in task prose must not masquerade as qualified code symbols
# and force unnecessary L2 context disclosure.
WO029_C2_WORK_ORDER = "WO-029-C2"
WO029_C2_BASE_SHA = "3d3478dcc5a73f50f13627cb3a806f0207f7cada"
WO029_C2_ALLOWED_PATHS = frozenset(
    {
...
HIVE_REL_007_WORK_ORDER = "HIVE-REL-007"
HIVE_REL_007_BASE_SHA = "da199d359d652fd0d6a2dd5f47442161715991fe"
HIVE_REL_007_ALLOWED_PATHS = frozenset(
    {
        ".engineering/release/HIVE-V1.0.2-SECURITY-TRIAGE.json",
        ".engineering/release/HIVE-V1.0.2-RELEASE-CANDIDATE.json",
        "VERSION",
        "backend/app/__init__.py",
        "backend/app/config.py",
        "dashboard/package.json",
...
HIVE_REL_007_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES

HIVE_REL_008_WORK_ORDER = "HIVE-REL-008"
HIVE_REL_008_BASE_SHA = "b95d5027c3975787cd01aa1921c57969b5963448"
HIVE_REL_008_ALLOWED_PATHS = frozenset(
    {
        ".engineering/release/HIVE-V1.0.2-PUBLISH-REQUEST.json",
        "scripts/review_evidence.py",
        "backend/tests/test_review_evidence.py",
    }
)
HIVE_REL_008_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES

HIVE_REL_009_WORK_ORDER = "HIVE-REL-009"
HIVE_REL_009_BASE_SHA = "05e48e74fe4655e49eda5699458d50282428ee40"
HIVE_REL_009_ALLOWED_PATHS = frozenset(
    {
        "VERSION",
        "backend/app/__init__.py",
        "backend/app/config.py",
...
HIVE_REL_009_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES

HIVE_REL_010_WORK_ORDER = "HIVE-REL-010"
HIVE_REL_010_BASE_SHA = "c924c9413c970ce5f0da5e6b0d9ed8fd5941afdb"
HIVE_REL_010_ALLOWED_PATHS = frozenset(
    {
...
HIVE_REL_010_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES

HIVE_REL_011_WORK_ORDER = "HIVE-REL-011"
HIVE_REL_011_BASE_SHA = "b67acc1f74347108ecbb783440cd7c1302962ea9"
HIVE_REL_011_ALLOWED_PATHS = frozenset(
    {
        ".github/workflows/release-publisher.yml",
        ".engineering/release/HIVE-V1.0.3-PUBLISH-REQUEST.json",
        "scripts/review_evidence.py",
...
HIVE_REL_011_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES

HIVE_REL_012_WORK_ORDER = "HIVE-REL-012"
HIVE_REL_012_BASE_SHA = "acdc22752180cb5173bb1e50a22eebc5e1a4757a"
HIVE_REL_012_ALLOWED_PATHS = frozenset(
    {
        "scripts/control_center_integration.py",
        "backend/tests/test_v01_closure_sprint.py",
        ".engineering/release/HIVE-V1.0.3-PUBLISH-REQUEST.json",
...
HIVE_REL_012_FORBIDDEN_PREFIXES = HIVE_REL_003_FORBIDDEN_PREFIXES
...
def require_hive_rel_007_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound v1.0.2 release preparation to its exact base and file surface."""

    if work_order != HIVE_REL_007_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_007_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_007_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_007_WORK_ORDER} requires exact base {HIVE_REL_007_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_007_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_007_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_007_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_007_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_007_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_007_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_007_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_007_WORK_ORDER} changes paths outside the bounded preparation scope: "
            + ", ".join(unauthorized)
        )


def require_hive_rel_008_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound v1.0.2 publication authorization to its exact base and file surface."""

    if work_order != HIVE_REL_008_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_008_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_008_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_008_WORK_ORDER} requires exact base {HIVE_REL_008_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_008_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_008_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_008_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_008_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_008_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_008_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_008_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_008_WORK_ORDER} changes paths outside the bounded publication scope: "
            + ", ".join(unauthorized)
        )


def require_hive_rel_009_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound v1.0.3 release preparation to its exact base and file surface."""

    if work_order != HIVE_REL_009_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_009_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_009_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_009_WORK_ORDER} requires exact base {HIVE_REL_009_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_009_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_009_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_009_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_009_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_009_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_009_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_009_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_009_WORK_ORDER} changes paths outside the bounded release-preparation "
            "scope: " + ", ".join(unauthorized)
        )


def require_hive_rel_010_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound v1.0.3 publication authorization to its exact base and file surface."""

    if work_order != HIVE_REL_010_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_010_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_010_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_010_WORK_ORDER} requires exact base {HIVE_REL_010_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_010_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_010_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_010_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_010_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_010_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_010_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_010_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_010_WORK_ORDER} changes paths outside the bounded publication scope: "
            + ", ".join(unauthorized)
        )


def require_hive_rel_011_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound the v1.0.3 publisher correction to its exact base and file surface."""

    if work_order != HIVE_REL_011_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_011_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_011_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_011_WORK_ORDER} requires exact base {HIVE_REL_011_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_011_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_011_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_011_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_011_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_011_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_011_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_011_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_011_WORK_ORDER} changes paths outside the bounded publisher correction: "
            + ", ".join(unauthorized)
        )


def require_hive_rel_012_scope(
    work_order: str,
    base_sha: str,
    paths: list[str],
    *,
    ...
    """Bound the v1.0.3 fixture-cleanup correction to its exact base and file surface."""

    if work_order != HIVE_REL_012_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{HIVE_REL_012_WORK_ORDER} requires the protected main base branch")
    if base_sha != HIVE_REL_012_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_012_WORK_ORDER} requires exact base {HIVE_REL_012_BASE_SHA}, "
            f"observed {base_sha}"
        )
    if authorized_base_sha != HIVE_REL_012_BASE_SHA:
        raise ValueError(
            f"{HIVE_REL_012_WORK_ORDER} requires the exact authorized-base marker "
            f"{HIVE_REL_012_BASE_SHA}"
        )
    if not paths:
        raise ValueError(f"{HIVE_REL_012_WORK_ORDER} requires a non-empty change set")
    unique_paths = set(paths)
    forbidden = sorted(
        path for path in unique_paths if path.startswith(HIVE_REL_012_FORBIDDEN_PREFIXES)
    )
    if forbidden:
        raise ValueError(
            f"{HIVE_REL_012_WORK_ORDER} cannot change canonical or local-only paths: "
            + ", ".join(forbidden)
        )
    unauthorized = sorted(unique_paths - HIVE_REL_012_ALLOWED_PATHS)
    if unauthorized:
        raise ValueError(
            f"{HIVE_REL_012_WORK_ORDER} changes paths outside the fixture-cleanup correction: "
            + ", ".join(unauthorized)
        )
...
def require_wo029_c2_scope(
    ...
    *,
    ...
    """Bound modern-domain disclosure correction to the exact post-WO-029-C1 main base."""

    if work_order != WO029_C2_WORK_ORDER:
        return
    if base_branch != "main":
        raise ValueError(f"{WO029_C2_WORK_ORDER} requires the protected main base branch")
    if base_sha != WO029_C2_BASE_SHA:
        raise ValueError(
            f"{WO029_C2_WORK_ORDER} requires exact base {WO029_C2_BASE_SHA}, observed {base_sha}"
        )
    if enforce_current_main:
        current_main = git_value("rev-parse", "origin/main", fallback="")
        if HEX_SHA.fullmatch(current_main) and current_main != WO029_C2_BASE_SHA:
            raise ValueError(
                f"{WO029_C2_WORK_ORDER} is stale: protected main is {current_main}, "
                f"authorized base is {WO029_C2_BASE_SHA}"
            )
    if len(paths) != len(WO029_C2_ALLOWED_PATHS) or set(paths) != WO029_C2_ALLOWED_PATHS:
        raise ValueError(
            f"{WO029_C2_WORK_ORDER} requires exactly the bounded domain-classification surface"
        )
    if enforce_authorized_base:
        if authorized_base_sha is None:
            raise ValueError(f"{WO029_C2_WORK_ORDER} requires exactly one authorized-base marker")
        if HEX_SHA.fullmatch(authorized_base_sha) is None:
            raise ValueError(
                f"{WO029_C2_WORK_ORDER} authorized-base marker must be lowercase 40-hex"
            )
        if authorized_base_sha != base_sha:
            raise ValueError(
                f"{WO029_C2_WORK_ORDER} authorized-base marker must match the pull request base SHA"
            )
