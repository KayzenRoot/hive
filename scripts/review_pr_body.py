...
AUTHORIZED_BASE_BY_WORK_ORDER = {
    ...
    "WO-025-G3": "7e52a5231b4bfadc54b3591f1ce3e3f22e40e66b",
}
...
Esta PR habilita exclusivamente o contrato de Review Evidence para o próximo
incremento Full HIVE Control Center. É uma mudança de governança: não
implementa backend, dashboard, migration, checkpoint, release ou o produto
WO-022.
...
Exatamente quatro arquivos: `backend/tests/test_review_evidence.py`,
`schemas/review-evidence-v1.schema.json`, `scripts/review_evidence.py` e
`scripts/review_pr_body.py`. Project Brain, checkpoint, migration
`0007_telemetry_events`, backend de produto, dashboard, dependências, CI e
release permanecem intocados. A PR não cria branch ou produto WO-022.

## 4. Contrato futuro separado

O produto futuro é registrado como `WO-022`, separado de `WO-022-G1`, com o
contrato `control-center-full-v1` e o arquivo `control-center-full.json`.
Após a inspeção do núcleo já entregue no WO-020 e das métricas do WO-021, o
menor incremento coerente restante cobre inteligência de projeto (checkpoint,
escopo, DoD, índice, commits, histórico de runs, decisões/memória,
módulos/símbolos, grafo de dependências, histórico de qualidade e qualidade de
retrieval), os gráficos obrigatórios, os alertas obrigatórios e as lacunas de
saúde de recursos/container/modelo local condicional.

## 5. Evidência e fail-closed

O futuro exige capacidades, gráficos, alertas e saúde enumerados sem campos
extras; payload/render bounded; escopo por projeto; refresh near-real-time;
PostgreSQL canônico; Redis não canônico; recuperação após restart e perda do
Redis; e proveniência explícita para valores estimados, indisponíveis e
desconhecidos. Métrica ou alerta fabricado, desconhecido convertido em zero,
leak, path absoluto ou chamada LLM/provider falha fechado. O contrato não
permite alegar HIVE V0.1 completo prematuramente.

## 6. Governança e STOP

Ruleset antes: {ruleset_before}. Ruleset depois: {ruleset_after}. Merge antes:
{merge_before}. Merge depois: {merge_after}. Auto-merge permanece UNARMED.
Não houve merge, release, promoção de checkpoint ou início do WO-022.

A PR permanece aberta, Ready e não mesclada. Sol Review State: AWAITING_SOL.
O executor para antes do merge e entrega o HEAD exato e o Evidence Bundle para
auditoria de Sol.

WO-022-G1 READY FOR SOL AUDIT
"""


def _render_wo022_body(
    *,
    work_order: str,
    pr_number: int,
    branch: str,
...
Esta PR futura implementa somente o incremento bounded restante do Full HIVE
Control Center registrado no G1. A base auditada é `{base_sha}` e o HEAD é
`{head_sha}`. O contrato não autoriza declarar HIVE V0.1 completo sem o restante
do Definition of Done.

## 2. Capacidades obrigatórias

O contrato `control-center-full-v1` usa `control-center-full.json` e fecha a
lista de inteligência de projeto, gráficos de tokens/contexto/cache/storage,
atividade/qualidade/latência, alertas de operação e saúde de recursos,
containers e modelos locais quando aplicável. O Evidence Bundle deve provar
cada item enumerado, com payloads bounded e paths repository-relative.

## 3. Verdade e proveniência

PostgreSQL é canônico e Redis é não canônico. Valores estimados,
UNAVAILABLE e UNKNOWN permanecem distintos; nenhuma métrica ou alerta pode ser
fabricado ou convertido em zero. O produto futuro exige escopo por projeto,
refresh near-real-time, replay/reconciliação e recuperação após restart/Redis
loss, sem migrations nesta fatia.
...
Esta PR é somente governança: habilita o Review Evidence para a promoção
canônica futura do Control Center operational core já aprovado no WO-020. Não
edita o Project Brain, não promove o checkpoint, não implementa produto e não
executa o WO-020-P.
...
## 4. Par ativo e contrato futuro

O único par ativo de promoção de checkpoint passa a ser WO-020-P-G1 e WO-020-P;
WO-019-P-G1/WO-019-P ficam históricos e promoções desconhecidas como WO-021-P e
WO-999-P continuam rejeitadas. O futuro WO-020-P permitirá somente
docs/project-brain/13-CHECKPOINT.md e
docs/project-brain/CANONICAL-SHA256SUMS.txt, com base autorizada, gramática de
checkpoint fechada e contrato exato do manifesto.

## 5. Evidência obrigatória

O Review Evidence desta G1 e do futuro WO-020-P exige a evidência já merged
`control-center-core-v1` a partir da main atual e falha fechado se estiver
ausente ou FAIL. Nenhum comportamento novo de produto é exigido.

## 6. Governança

Ruleset antes: {ruleset_before}. Ruleset depois: {ruleset_after}. Merge antes:
{merge_before}. Merge depois: {merge_after}. Auto-merge permanece UNARMED.
Não houve merge, release, promoção de checkpoint ou início do WO-020-P.

A PR permanece aberta, Ready e não mesclada. Sol Review State: AWAITING_SOL.

WO-020-P-G1 READY FOR SOL AUDIT
"""


def _render_wo020p_body(
    *,
    work_order: str,
    pr_number: int,
    branch: str,
...
## 1. Promoção Control Center operational core fechada

Esta PR promove somente o checkpoint do WO-020 já aprovado, merged e validado
...
STATUS muda para CONTROL CENTER OPERATIONAL CORE APPROVED / V0.1 IMPLEMENTATION
ACTIVE. O prefixo histórico de COMPLETED é preservado, seis bullets de evidência
WO-020 são anexadas, `full Control Center.` permanece em PENDING com todos os
demais itens e ordem, e IN PROGRESS, BLOCKERS e NEXT STEP passam para a menor
implementação de métricas e observabilidade do Control Center.

## 3. Evidência e manifesto

O contrato `control-center-core-v1` permanece obrigatório e verde; nenhum
produto novo é reivindicado. Somente o digest do checkpoint em
...
o executor para antes do merge.

Sol Review State: AWAITING_SOL.

WO-020-P READY FOR SOL AUDIT
"""
...
Esta PR é somente governança: habilita o Review Evidence para a futura
promoção canônica do Full HIVE Control Center entregue no WO-022, já aprovado,
merged e validado no pós-merge. Não edita o Project Brain, não promove o
checkpoint, não implementa produto e não executa o WO-022-P.
...
Esta G1 e o futuro WO-022-P exigem a evidência já merged
`control-center-full-v1`, o lineage exato do PR #86, o HEAD auditado
a870275b3ad738b789e30169d5b1fac00e2e4ae3 e a migration head
`0007_telemetry_events`. A promoção registra dez capabilities, treze charts,
doze alertas e três superfícies de health, mantém PostgreSQL canônico e Redis
não canônico, e não autoriza alegação de HIVE V0.1 completo.

## 6. Governança

Ruleset antes: {ruleset_before}. Ruleset depois: {ruleset_after}. Merge antes:
{merge_before}. Merge depois: {merge_after}. Auto-merge permanece UNARMED.
Não houve merge, release, promoção de checkpoint ou início do WO-022-P.

A PR permanece aberta, Ready e não mesclada. Sol Review State: AWAITING_SOL.

WO-022-P-G1 READY FOR SOL AUDIT
"""


def _render_wo022p_body(
    *,
    work_order: str,
    pr_number: int,
    branch: str,
...
## 1. Promoção Full HIVE Control Center fechada

Esta PR promove somente o checkpoint do WO-022 já aprovado, merged e validado
no pós-merge, sem alegar HIVE V0.1 completo. Exige exatamente
`docs/project-brain/13-CHECKPOINT.md` e
`docs/project-brain/CANONICAL-SHA256SUMS.txt`.
...
STATUS muda para FULL CONTROL CENTER APPROVED / V0.1 IMPLEMENTATION ACTIVE. O
prefixo histórico de COMPLETED é preservado e seis bullets fechados de evidência
WO-022 são anexados. Apenas `full Control Center.` sai de PENDING, preservando
benchmarks abrangentes de retrieval/token/storage, estabilização, deployment
local, backup/recovery, documentação, revisão final e checkpoint awareness na
ordem canônica. IN PROGRESS e NEXT STEP avançam para os benchmarks abrangentes
de retrieval/token/storage; não repetem o incremento concluído do Full Control
Center.

## 3. Evidência e manifesto

O contrato `control-center-full-v1` e o lineage exato do PR #86 permanecem
obrigatórios e verdes. Seções obrigatórias de governança ausentes falham
fechadas como UNAVAILABLE, valores desconhecidos nunca são renderizados como
zero e nenhuma métrica ou alerta é fabricado. Somente o digest do checkpoint em
`CANONICAL-SHA256SUMS.txt` pode mudar, correspondendo aos bytes exatos do
candidato. A migration head permanece `0007_telemetry_events`.
...
def render_body(
    *,
    ...
    if work_order == "WO-022-G1":
        return _render_wo022_g1_body(
            work_order=work_order,
            pr_number=pr_number,
            branch=branch,
            base_sha=base_sha,
    ...
    if work_order == "WO-022":
        return _render_wo022_body(
            work_order=work_order,
            pr_number=pr_number,
            branch=branch,
            base_sha=base_sha,
    ...
    if work_order == "WO-020-P-G1":
        return _render_wo020p_g1_body(
            work_order=work_order,
            pr_number=pr_number,
            branch=branch,
            base_sha=base_sha,
    ...
    if work_order == "WO-022-P":
        return _render_wo022p_body(
            work_order=work_order,
            pr_number=pr_number,
            branch=branch,
            base_sha=base_sha,
