...
def _provenance(event: EventEnvelope, key: str) -> MetricProvenance:
    ...
    if (
        event.payload.get("provider_final_usage") is True
...
def _token_metrics(events: list[EventEnvelope]) -> dict[str, MetricValue]:
    selected: dict[
        tuple[str, str],
        tuple[float | int, MetricProvenance, datetime],
    ] = {}
    rank = {
        MetricProvenance.EXACT: 3,
        MetricProvenance.ESTIMATED: 2,
...
def _context_metrics(events: list[EventEnvelope]) -> dict[str, MetricValue]:
    ...
    return {
        ...
        "reduction_tokens": _value(
            reduction,
            provenance,
            "derived:context-reduction",
        ),
        "reduction_ratio": (
            _value(
                reduction / before_total,
                provenance,
                "derived:context-reduction-ratio",
            )
            if before_total > 0
...
def _cache_metrics(events: list[EventEnvelope]) -> dict[str, MetricValue]:
    ...
    if total == 0:
        return {
            name: _missing("canonical-cache-events")
            for name in (
                "hits",
                "misses",
                "observed_decisions",
...
def _storage_metrics(
    ...
    return {
        "logical_task_bytes": _value(
            logical,
            MetricProvenance.EXACT,
            "postgres:tasks.logical_size",
        ),
        "physical_referenced_bytes": _value(
            physical,
            MetricProvenance.EXACT,
            "postgres:cas_blobs.physical_size",
        ),
        "saved_bytes": _value(
            saved,
            MetricProvenance.EXACT,
            "derived:logical-physical",
        ),
        "savings_ratio": (
            _value(
                saved / logical,
                MetricProvenance.EXACT,
                "derived:storage-savings-ratio",
            )
            if logical > 0
        ...
        "referenced_blob_count": _value(
            observation.referenced_blob_count,
            MetricProvenance.EXACT,
            "postgres:tasks.original_blob_sha256",
        ),
    }
...
def _global_snapshot(
    ...
    def family(
        ...
        return {
            name: _aggregate(
                [getattr(snapshot, attr)[name] for snapshot in snapshots],
                f"global:{attr}:{name}",
            )
            for name in names
        }
    ...
    if before.value is not None and after.value is not None:
        ...
        context["reduction_tokens"] = _value(
            reduction,
            provenance,
            "global:context:reduction",
        )
        context["reduction_ratio"] = (
            _value(
                reduction / before.value,
                provenance,
    ...
    cache = family(
        ("hits", "misses", "observed_decisions"),
        "cache",
    )
...
def control_center_metrics(
    project_id: UUID,
    history_points: int = Query(default=100, ge=1, le=100),
) -> MetricsResponse:
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="control center metrics database unavailable",
        ) from exc


@router.get(
    "/api/v1/control-center/metrics/global",
    response_model=MetricsResponse,
)
def control_center_metrics_global(
    history_points: int = Query(default=100, ge=1, le=100),
) -> MetricsResponse:
    try:
