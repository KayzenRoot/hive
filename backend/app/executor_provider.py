...
def _result(payload: object, model: str, request_id: str | None) -> ExecutorResult:
    ...
    def execute(self, request: ExecutorRequest, context: object) -> ExecutorResult:
        ...
        if self.settings.executor_api_key is not None:
            headers["Authorization"] = (
                f"Bearer {self.settings.executor_api_key.get_secret_value()}"
            )
        http_request = Request(self.url, data=data, headers=headers, method="POST")
        try:
            with urlopen(http_request, timeout=self.settings.executor_timeout_seconds) as response:
        ...
        if not isinstance(decoded, dict) or not isinstance(choices, list) or not isinstance(message, str):
        ...
        return _result(structured, self.model, request_id)
