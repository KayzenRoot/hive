...
def test_hive_rel_010_v103_publish_authorization_scope_is_exact_and_fail_closed() -> None:
    expected_paths = frozenset(
        {
    ...
    assert expected_paths == HIVE_REL_010_ALLOWED_PATHS
    assert HIVE_REL_010_WORK_ORDER in review_evidence.REGISTERED_WORK_ORDERS
    assert HIVE_REL_010_WORK_ORDER in review_evidence.AUTHORIZED_BASE_MARKER_WORK_ORDERS
    require_supported_work_order(HIVE_REL_010_WORK_ORDER)
    require_current_work_order_authorization(HIVE_REL_010_WORK_ORDER)

    body = (
        f"<!-- HIVE-WORK-ORDER: {HIVE_REL_010_WORK_ORDER} -->\n"
        f"<!-- HIVE-AUTHORIZED-BASE: {HIVE_REL_010_BASE_SHA} -->"
    )
    assert parse_work_order_marker(body) == HIVE_REL_010_WORK_ORDER
    assert (
        review_evidence.authorized_base_marker_sha(HIVE_REL_010_WORK_ORDER, body)
        == HIVE_REL_010_BASE_SHA
    )
    with pytest.raises(ValueError, match="unsupported release-engineering work order"):
        require_current_work_order_authorization("HIVE-REL-012")
    ...
    with pytest.raises(ValueError, match="authorized-base marker"):
        require_hive_rel_010_scope(HIVE_REL_010_WORK_ORDER, HIVE_REL_010_BASE_SHA, allowed)
    with pytest.raises(ValueError, match="authorized-base marker"):
        require_hive_rel_010_scope(
            HIVE_REL_010_WORK_ORDER,
            HIVE_REL_010_BASE_SHA,
...
def test_hive_rel_011_publisher_correction_scope_is_exact_and_fail_closed() -> None:
    ...
    assert expected_paths == HIVE_REL_011_ALLOWED_PATHS
    assert HIVE_REL_011_WORK_ORDER in review_evidence.REGISTERED_WORK_ORDERS
    assert HIVE_REL_011_WORK_ORDER in review_evidence.AUTHORIZED_BASE_MARKER_WORK_ORDERS
    require_supported_work_order(HIVE_REL_011_WORK_ORDER)
    require_current_work_order_authorization(HIVE_REL_011_WORK_ORDER)

    body = (
        f"<!-- HIVE-WORK-ORDER: {HIVE_REL_011_WORK_ORDER} -->\n"
        f"<!-- HIVE-AUTHORIZED-BASE: {HIVE_REL_011_BASE_SHA} -->"
    )
    assert parse_work_order_marker(body) == HIVE_REL_011_WORK_ORDER
    assert (
        review_evidence.authorized_base_marker_sha(HIVE_REL_011_WORK_ORDER, body)
        == HIVE_REL_011_BASE_SHA
    )
    with pytest.raises(ValueError, match="unsupported release-engineering work order"):
        require_current_work_order_authorization("HIVE-REL-012")
    ...
    require_hive_rel_011_scope(
        ...
        HIVE_REL_011_BASE_SHA,
        allowed,
        authorized_base_sha=HIVE_REL_011_BASE_SHA,
    )
    with pytest.raises(ValueError, match="protected main"):
        require_hive_rel_011_scope(
            HIVE_REL_011_WORK_ORDER,
    ...
    with pytest.raises(ValueError, match="authorized-base marker"):
        require_hive_rel_011_scope(HIVE_REL_011_WORK_ORDER, HIVE_REL_011_BASE_SHA, allowed)
    with pytest.raises(ValueError, match="authorized-base marker"):
        require_hive_rel_011_scope(
            HIVE_REL_011_WORK_ORDER,
            HIVE_REL_011_BASE_SHA,
...
def test_hive_rel_011_publisher_environment_and_request_rebinding() -> None:
    workflow_path = review_evidence.ROOT / ".github" / "workflows" / "release-publisher.yml"
    workflow_lines = workflow_path.read_text(encoding="utf-8").splitlines()
    publish_start = workflow_lines.index("  publish:")
    next_job = next(
        (
            index
            for index in range(publish_start + 1, len(workflow_lines))
            if re.fullmatch(r"  [A-Za-z_][A-Za-z0-9_-]*:", workflow_lines[index])
        ),
        len(workflow_lines),
    )
    publish_job = workflow_lines[publish_start:next_job]
    env_start = publish_job.index("    env:")
    steps_start = publish_job.index("    steps:")
    publish_env = publish_job[env_start + 1 : steps_start]
    compose_assignments = [
        line.partition(":")[2].strip().strip("\"'")
        for line in publish_env
        if line.startswith("      COMPOSE_PROJECT_NAME:")
    ]
    assert len(compose_assignments) == 1
    assert compose_assignments[0]
    assert "wo031" in compose_assignments[0].casefold()
